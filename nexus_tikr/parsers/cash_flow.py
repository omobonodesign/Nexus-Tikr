"""Template C: TIKR_CASH_FLOW parser."""
...
from nexus_tikr.constants import CASH_FLOW_LABELS
...
def parse_cash_flow(
    ...
    """Parse Cash Flow file using semantic label matching."""
    ...
    for field_name, (label_key, parse_type, impact) in CASH_FLOW_LABELS.items():
        row = engine.find_label(
            label_key,
            file_type="TIKR_CASH_FLOW",
            impact=impact,
        )
        if row is not None:
            mapped_rows.add(row)
